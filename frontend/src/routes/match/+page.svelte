  ...
  async function handleStartMatchmaking() {
    ...
    try {
      ...
      const errMsg = err instanceof Error
        ? err.message
        : 'Error memulai random match, silakan coba lagi.';
...
<div class="h-screen flex flex-col bg-[#F4F6F9] text-slate-800 font-sans select-none overflow-hidden">
  <!-- Header -->
  <header class="h-16 flex-shrink-0 flex items-center justify-between px-8 border-b border-slate-200/50 bg-white shadow-sm">
    <!-- Title -->
    <div class="flex items-center space-x-2.5">
      <img src="/logo.webp" alt="UMBRA Logo" class="w-8 h-8 object-contain rounded-lg" />
      <span class="font-bold text-lg text-[#00658D] tracking-tight font-['Space_Grotesk']">UMBRA</span>
    </div>

    <!-- Status Badge -->
    <div class="flex items-center space-x-2 bg-[#E6F4EA] text-[#137333] px-3.5 py-1.5 rounded-full text-xs font-semibold">
  ...
  <div class="flex-grow flex flex-col items-center justify-center p-6 max-w-lg mx-auto w-full text-center space-y-12">
    ...
    <div class="flex items-center justify-between w-full max-w-sm relative">
      ...
      <div class="flex flex-col items-center space-y-3 z-10">
        <div class="w-20 h-20 rounded-full border-4 border-[#00aeef] bg-white flex items-center justify-center shadow-lg relative">
        ...
        <span class="text-xs font-bold text-[#00aeef] tracking-widest uppercase font-mono">ANDA</span>
      </div>

      <!-- Right Avatar (MENCARI) -->
      <div class="flex flex-col items-center space-y-3 z-10">
        <div class="w-20 h-20 rounded-full bg-[#F1F5F9] border-4 border-white flex items-center justify-center text-slate-400 font-bold text-3xl shadow-sm animate-pulse">
          ?
        </div>
        <span class="text-xs font-bold text-slate-400 tracking-widest uppercase font-mono">MENCARI...</span>
      </div>
    </div>
    ...
    <div class="w-full bg-[#0F1C2C] text-[#CBD5E1] py-3.5 px-6 rounded-full flex items-center justify-between text-[11px] font-bold font-mono shadow-md">
