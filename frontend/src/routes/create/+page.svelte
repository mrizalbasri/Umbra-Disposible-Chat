<script lang="ts">
	...
	function handleCreate() {
		...
		const query = new URLSearchParams({ tab: 'create', nickname: finalNickname }).toString();
		goto(resolve(`/chatroom?${query}`));
	}
</script>
...
<div class="layout">
	...
	<aside class="sidebar">
		<div class="sidebar-top">
			<!-- Logo -->
			<div class="brand">
				<img src="/logo.webp" alt="UMBRA Logo" class="logo-img" style="height: 32px; width: 32px;" />
				<span class="brand-text">UMBRA</span>
			</div>

			<!-- Tombol Kembali -->
			<button class="btn-back" onclick={() => goto(resolve('/'))}> ← KEMBALI </button>
			...
			<div class="guide">
				<h3>PANDUAN SESI</h3>
				<p>
					Umbra Protocol menjamin enkripsi end-to-end pada setiap transmisi. Room akan otomatis
					hancur setelah semua peserta keluar atau sesi berakhir.
				</p>
			</div>
		</div>
		...
		<div class="form-card">
			...
			<div class="input-group">
				<div class="input-labels">
					<label for="nickname-input">NICKNAME (OPSIONAL)</label>
				...
				<div class="input-wrapper">
					...
					<button
						type="button"
						class="btn-random"
						aria-label="Generate nickname acak"
			...
			<div class="join-link">
				Join ke room teman ?
				<button type="button" class="join-link-btn" onclick={() => window.location.assign('/join')}
					>Klik disini</button
				>
			</div>
		</div>
